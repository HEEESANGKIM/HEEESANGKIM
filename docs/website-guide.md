...
public/
  favicon.svg                 # HK 파비콘
  heesang-kim.jpg              # 제공된 About 프로필 사진
  .nojekyll
...
src/
  ...
  layout.css                  # 데스크톱 한 화면 섹션, About 사진·학력 배치
  App.tsx
  main.tsx
...
About의 소개·학력은 제공된 CV를 바탕으로 작성했으며, 졸업 예정일은 소유자가 확인한 **2027년 2월**입니다. 뉴욕 본교 수학은 별도 학위가 아닌 동일 학사 과정의 수학 이력으로 표시합니다. `research`, `publications`, `projects`, `experience`와 기술 항목은 아직 비어 있습니다. 연구 **관심 분야** 문구는 기존 내용을 유지합니다.

데스크톱(너비 1120px 이상)은 각 섹션을 내비게이션 아래 한 화면 높이에 맞춥니다. 내용이 많아지면 섹션이 늘어나며, 모바일과 확대 화면에서는 일반 스크롤 레이아웃을 사용합니다. 조정 위치는 `src/layout.css`입니다.

[콘텐츠 입력 예시](content-guide.md)를 참고하세요.
...
- 히어로는 승용차와 차량 소프트웨어·중앙 컴퓨팅 구조를 연결한 SDV·자율주행 SVG입니다. Research는 SDV 구조 → 카메라 인식 장면 → Edge AI 보드 순서로 표시합니다. `SystemVisual.tsx`와 `components/system-visual.css`에서 편집합니다. 개념 참고: [NVIDIA Robotics](https://www.nvidia.com/en-us/autonomous-machines/), [Bosch SDV](https://www.bosch-mobility.com/en/mobility-topics/software-defined-vehicle/). 외부 이미지를 복제하거나 불러오지 않습니다.
- About의 원본 사진은 `public/heesang-kim.jpg`입니다. `profile.portrait`에서 경로·대체 텍스트·원본 치수를 관리합니다. 사진 영역에서 우클릭 메뉴·드래그·모바일 길게 누르기 메뉴를 제한합니다. 이 동작은 일반적인 저장을 억제하는 UI 처리이며 접근 권한 제어가 아닙니다. 공개 사이트/저장소의 이미지 URL 접근과 화면 캡처를 완전히 막을 수는 없습니다. 사진은 지연 로딩되며, CV 원본 PDF는 공개 폴더에 포함하지 않습니다.
